...
export default function AnalyticsPage() {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      setMonitoringDocs((data as LhuDocument[]) || []);
    } catch (err) {
      console.error('Gagal mengambil berkas untuk analitik:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className={`bg-theme-card border-t-2 ${isDirektur ? 'border-t-purple-500' : 'border-t-rose-500'} border-x border-b border-theme-border rounded-2xl p-6 shadow-xl relative`}>
        {loading ? (
          <div className="text-center py-12 text-theme-dim text-xs font-semibold animate-pulse">
            Memuat grafik analisis data...
          </div>
        ) : (
          <AnalyticsTab monitoringDocs={monitoringDocs} />
        )}
      </div>
    </div>
  );
